 ...
 *
 */
public class IFscBankWatchListDimDaoImpl implements IFscBankWatchListDimDaoCustom {
	...
	@Override
	public List<FscBankWatchListDim> nativeSql(String sql) {
		
		Query query = entityManager.createNativeQuery(sql, FscBankWatchListDim.class);
		List<FscBankWatchListDim> resultList = (List<FscBankWatchListDim>)query.getResultList();
...
//		String select = "   SELECT ENTITY_WATCH_LIST_KEY, ENTITY_NAME, IDENTIFICATION_ID, CITIZENSHIP_COUNTRY_CODE, YEAR_OF_BIRTH, WATCH_LIST_NAME, CATEGORY_DESC   ";
		...
		String from = String.format("   FROM %s.%s.FSC_BANK_WATCH_LIST_DIM   ", AmlConfiguration.getAmlCatalog(), AmlConfiguration.getFcfcoreSchema());
