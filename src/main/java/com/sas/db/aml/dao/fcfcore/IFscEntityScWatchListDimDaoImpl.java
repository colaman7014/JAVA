 ...
 *
 */
public class IFscEntityScWatchListDimDaoImpl implements IFscEntityScWatchListDimDaoCustom {
	...
	@Override
	public List<FscEntityScWatchListDim> nativeSql(String sql) {
		
		Query query = entityManager.createNativeQuery(sql, FscEntityScWatchListDim.class);
		List<FscEntityScWatchListDim> resultList = (List<FscEntityScWatchListDim>)query.getResultList();
...
//		String select = "   SELECT ENTITY_WATCH_LIST_KEY, ENTITY_NAME, IDENTIFICATION_ID, CITIZENSHIP_COUNTRY_CODE, YEAR_OF_BIRTH, WATCH_LIST_NAME, CATEGORY_DESC   ";
		...
		String from = String.format("   FROM %s.%s.FSC_ENTITY_SC_WATCH_LIST_DIM   ", AmlConfiguration.getAmlCatalog(), AmlConfiguration.getFcfcoreSchema());
