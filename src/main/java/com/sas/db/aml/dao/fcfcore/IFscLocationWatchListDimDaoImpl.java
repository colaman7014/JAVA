 ...
 *
 */
public class IFscLocationWatchListDimDaoImpl implements IFscLocationWatchListDimDaoCustom {
	...
	@Override
	public List<FscLocationWatchListDim> nativeSql(String sql) {
		
		Query query = entityManager.createNativeQuery(sql, FscLocationWatchListDim.class);
		List<FscLocationWatchListDim> resultList = (List<FscLocationWatchListDim>)query.getResultList();
...
//		String select = "   SELECT ENTITY_WATCH_LIST_KEY, ENTITY_NAME, IDENTIFICATION_ID, CITIZENSHIP_COUNTRY_CODE, YEAR_OF_BIRTH, WATCH_LIST_NAME, CATEGORY_DESC   ";
		...
		String from = String.format("   FROM %s.%s.FSC_LOCATION_WATCH_LIST_DIM   ", AmlConfiguration.getAmlCatalog(), AmlConfiguration.getFcfcoreSchema());
