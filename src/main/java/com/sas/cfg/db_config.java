...
public class db_config {
    ...
    synchronized static Properties getRS() {
        _rs = _rs == null ? convertToProperties(ResourceBundle.getBundle(CFG)) :
              _rs;
        return _rs;
    }
    ...
    public static boolean isDBJndiLocType() {
        _db_connect_type = (_db_connect_type == null) ?
                           getRS().getProperty(DB_CONNECT_TYPE) :
                           _db_connect_type;
        return JDBC_JNDI_LOC.equals(_db_connect_type);
    }

    public static String getJndiLocInfo() {
        _jdbc_jndi_loc = (_jdbc_jndi_loc == null) ?
                         getRS().getProperty(JDBC_JNDI_LOC) :
                         _jdbc_jndi_loc;
