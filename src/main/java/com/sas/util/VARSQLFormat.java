...
/**
 * <code>DOSQLFormat</code> provides a means to produce concatenated
 * messages in language-neutral way. Use this to construct messages
 * displayed for end users.
 *
 * <p>
 * <code>DOSQLFormat</code> takes a set of objects, formats them, then
 * inserts the formatted strings into the pattern at the appropriate places.
 *
 * <p>
 * <strong>Note:</strong>
 * <code>DOSQLFormat</code> differs from the other <code>Format</code>
 * classes in that you create a <code>DOSQLFormat</code> object with one
 * of its constructors (not with a <code>getInstance</code> style factory
 * method). The factory methods aren't necessary because <code>DOSQLFormat</code>
 * doesn't require any complex setup for a given locale. In fact,
 * <code>DOSQLFormat</code> doesn't implement any locale specific behavior
 ...
 * Inside a <code>DOSQLFormatElement</code>, quotes are <strong>not</strong>
 ...
 * by a <code>ChoiceFormat</code> in <code>DOSQLFormat</code> is treated specially;
 * occurances of '{' are used to indicated subformats, and cause recursion.
 * If you create both a <code>DOSQLFormat</code> and <code>ChoiceFormat</code>
...
public class VARSQLFormat {
    ...
    /**
     * Convenience routine.
     * Avoids explicit creation of DOSQLFormat,
     * but doesn't allow future optimizations.
     */
    final public static PreparedStatement prepareDBStatement(String sqlPattern,
            Connection conn, Map valus) throws SQLException {
        return new VARSQLFormat(sqlPattern).prepareStatement(conn, valus);
    }

    final public static PreparedStatement prepareDBStatementNotlog(String sqlPattern,
            Connection conn, Map valus) throws SQLException {
    ...
    final public void applyPattern(String sqlPattern) {
        ...
        /*ArrayList al = new ArrayList();
                 StringReader sr = new StringReader(sqlPattern);
            ...
            while (true) {
                ...
                try {
                    ...
                    throw new NullPointerException(
                            "Can not read SQL pattern. [" + ex.getMessage() +
                            "]");
                }
                if (i == -1) {
                    break;
            ...
            sr.close();
                 ...
                 this.keys = new String[al.size()];
                 al.toArray(keys);*/
    }

    final public void addBatch(PreparedStatement psm, Map values) throws
            SQLException {
        bindPrepareStatemant(psm, values);
        psm.addBatch();
    }

    private void bindPrepareStatemant(PreparedStatement sm, Map values) throws
            SQLException {
        for (int i = 0, j = keys.length; i < j; i++) {
            ...
            if (values == null || (!values.containsKey(key))) {
                throw new IllegalArgumentException("Can not find column[" + key +
                        "] in VARSQL.\nSQL:" +
                        pattern);
            }
...
//            } else {
     ...
     * @return
     */
    final public PreparedStatement prepareStatement(Connection conn,
    ...
    final public PreparedStatement prepareStatementNotlog(Connection conn,
            Map values) throws SQLException {
        PreparedStatement sm = conn.prepareStatement(sql);
        bindPrepareStatemant(sm, values);
        return sm;
