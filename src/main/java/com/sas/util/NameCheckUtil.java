...
public class NameCheckUtil {
	...
	public static boolean indLongStr(String str){
		boolean result = false;
		int longStrNumber = Integer.parseInt(AmlConfiguration.getString("com.sas.swiftCheck.indLongStrNumber"));
		if(str != null && str.length() > 0){
			result = str.replaceAll(" ", "").length() >= longStrNumber;
		}
		return result;
	}
	
	public static boolean indLongTokenStr(String str){
		boolean result = false;
		int longTokenNumber = Integer.parseInt(AmlConfiguration.getString("com.sas.swiftCheck.indLongTokenStrNumber"));
		if(str != null && str.length() > 0){
			String strArray[] = str.split(" ");
			result = strArray.length >= longTokenNumber;
		}
		return result;
	}
	...
	public static void setToDestinationMap(Map<String, String> sourceMap, Map<String, String> destinationMap, Map<String, List<XWatchlistCompressString>> xWatchlistCompressStringMap){
		for (Map.Entry<String, String> entry : sourceMap.entrySet()) {
    		...
    		if((indFieldValue != null && indFieldValue.length() > 0) && !(NameCheckUtil.indLongStr(indFieldValue) || NameCheckUtil.indLongTokenStr(indFieldValue))){
    			setSortToDestinationMap(key, indFieldValue, destinationMap);
			}
    		if(orgFieldValue != null && orgFieldValue.length() > 0){
    			setSortToDestinationMap(key, orgFieldValue, destinationMap);
			}
    		if((sigFieldValue != null && sigFieldValue.length() > 0) && !(NameCheckUtil.indLongStr(sigFieldValue) || NameCheckUtil.indLongTokenStr(sigFieldValue))){
				setEmptyToDestinationMap(key, sigFieldValue, destinationMap);
			}
			
			setToDestinationMap(key, value, destinationMap);
		}
		logger.info("destinationMap : " + destinationMap.size());
	}
	 ...
	 * @param destinationMap
	 */
	public static void setToDestinationListMap(Map<String, String> sourceMap, Map<String, List<String>> destinationListMap, Map<String, List<XWatchlistCompressString>> xWatchlistCompressStringMap){
		for (Map.Entry<String, String> entry : sourceMap.entrySet()) {
			String key = entry.getKey();
			String value = StringUtils.lianxuStr(StringUtils.convertToHalfWidth(entry.getValue())).toUpperCase();//WatchListUtil.convertWordToSpace(entry.getValue(), SwiftMtConst.COVERTWORD);
			String indFieldValue = StringUtils.CompressStringByEntityType(SwiftMtConst.ENTITY_TYPE_IND, value, null, xWatchlistCompressStringMap).get("en");
    		...
    		if((indFieldValue != null && indFieldValue.length() > 0) && !(NameCheckUtil.indLongStr(indFieldValue) || NameCheckUtil.indLongTokenStr(indFieldValue))){
    			setSortToDestinationListMap(key, indFieldValue, destinationListMap);
			}
    		if(orgFieldValue != null && orgFieldValue.length() > 0){
    			setSortToDestinationListMap(key, orgFieldValue, destinationListMap);
			}
    		if((sigFieldValue != null && sigFieldValue.length() > 0) && !(NameCheckUtil.indLongStr(sigFieldValue) || NameCheckUtil.indLongTokenStr(sigFieldValue))){
    			setEmptyToDestinationListMap(key, sigFieldValue, destinationListMap);
			}
			
    		setToDestinationListMap(key, value, destinationListMap);
		}
		logger.info("destinationListMap : " + destinationListMap.size());
	}
	
	private static void setEmptyToDestinationMap(String key, String value, Map<String, String> destinationMap){
	...
	private static void setEmptyToDestinationListMap(String key, String value, Map<String, List<String>> destinationMap){
		int limitIndex = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_ARRANGEMENTSELECT_LIMITINDEX));
		int maxIndex = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_ARRANGEMENTSELECT_MAXINDEX));
		int fieldMaxLength = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_ARRANGEMENTSELECT_FIELDMAXLENGTH));
		...
		if(value != null){
			if(value.contains(" ")){
				for(String str : WatchListUtil.getShiftInMaxArrangementInLimitList(value.split(" "), maxIndex, limitIndex)){
					if(str != null && str.length() > 0 && str.length() <= fieldMaxLength){
						//compress all space to a word
						if(destinationMap.get(str.replace(" ", "")) != null){
							destinationMap.get(str.replace(" ", "")).add(key);
						}else{
							List<String> strList = new ArrayList<String>();
							strList.add(key);
							destinationMap.put(str.replace(" ", ""), strList);
						}
					}
				}
			}else{
				if(value.length() <= fieldMaxLength){
					if(destinationMap.get(value) != null){
						destinationMap.get(value).add(key);
					}else{
						List<String> strList = new ArrayList<String>();
						strList.add(key);
						destinationMap.put(value, strList);
					}
				}
			}
		}
	...
	private static void setSortToDestinationMap(String key, String value, Map<String, String> destinationMap){
		int limitIndex = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_ARRANGEMENTSELECT_LIMITINDEX));
		int maxIndex = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_ARRANGEMENTSELECT_MAXINDEX));
		int fieldMaxLength = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_ARRANGEMENTSELECT_FIELDMAXLENGTH));
		...
		if(value != null){
			if(value.contains(" ")){
				for(String str : WatchListUtil.getShiftInMaxArrangementInLimitList(value.split(" "), maxIndex, limitIndex)){
//					logger.debug(str + " sort : " + StringUtils.sortString(str));
					if(str != null && str.length() > 0 && str.length() <= fieldMaxLength){
	...
	private static void setSortToDestinationListMap(String key, String value, Map<String, List<String>> destinationMap){
		int limitIndex = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_ARRANGEMENTSELECT_LIMITINDEX));
		int maxIndex = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_ARRANGEMENTSELECT_MAXINDEX));
		int fieldMaxLength = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_ARRANGEMENTSELECT_FIELDMAXLENGTH));
...
//					logger.debug(str + " sort : " + StringUtils.sortString(str));
					if(str != null && str.length() > 0 && str.length() <= fieldMaxLength){
						if(destinationMap.get(StringUtils.sortString(str)) != null){
							destinationMap.get(StringUtils.sortString(str)).add(key);
						}else{
							List<String> strList = new ArrayList<String>();
							strList.add(key);
							destinationMap.put(StringUtils.sortString(str), strList);
						}
					}
				}
			}else{
	...
	private static void setToDestinationListMap(String key, String value, Map<String, List<String>> destinationMap){
		int limitIndex = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_ARRANGEMENTSELECT_LIMITINDEX));
		int maxIndex = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_ARRANGEMENTSELECT_MAXINDEX));
		int fieldMaxLength = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_ARRANGEMENTSELECT_FIELDMAXLENGTH));
		...
		if(value != null){
			if(value.contains(" ")){
				for(String str : WatchListUtil.getShiftInMaxArrangementInLimitList(value.split(" "), maxIndex, limitIndex)){
					if(str != null && str.length() > 0 && str.length() <= fieldMaxLength){
						if(destinationMap.get(str) != null){
							destinationMap.get(str).add(key);
						}else{
							List<String> strList = new ArrayList<String>();
							strList.add(key);
							destinationMap.put(str, strList);
						}
	 ...
	 * @param destinationMap
	 */
	public static void setToDestinationListMap(Map<String, String> sourceMap, Map<String, List<String>> destinationListMap){
		for (Map.Entry<String, String> entry : sourceMap.entrySet()) {
			String key = entry.getKey();
			String value = WatchListUtil.convertWordToSpace(entry.getValue(), SwiftMtConst.COVERTWORD);
			int limitIndex = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_ARRANGEMENTSELECT_LIMITINDEX));
			...
			if(value != null){
				if(value.contains(" ")){
					for(String str : WatchListUtil.getShiftInMaxArrangementInLimitList(value.split(" "), maxIndex, limitIndex)){
						if(str != null && str.length() > 0 && str.length() < fieldMaxLength){
							if(destinationListMap.get(str) != null){
								destinationListMap.get(str).add(key);
							}else{
								List<String> strList = new ArrayList<String>();
								strList.add(key);
								destinationListMap.put(str, strList);
							}
						}
					}
				}else{
					if(value.length() < fieldMaxLength){
						if(destinationListMap.get(value) != null){
							destinationListMap.get(value).add(key);
						}else{
							List<String> strList = new ArrayList<String>();
							strList.add(key);
							destinationListMap.put(value, strList);
						}
					}
				}
			}
...
//			float hitWordRatio = calculateHitWordRatio( nameHitRecord.getEntityName(), inputCnName, inputEnName, DLParameter); //加入差異度比較
	...
	public static float calculateHitWordRatio(String entity_name, String inputCnName, String inputEnName, int DLParameter , String entityType){
		...
		if(!StringUtils.isEnglish(entity_name)){	//not english
			if(inputCnName != null && inputCnName.length() > 0){ 
				if(inputCnName.length() <= SwiftMtConst.INCLUSIVE_MATCH_CH_NAME_TOKEN_5){  //五個中文字以下才做全姓名相同字數置換精準比對
					if(entity_name.length() == inputCnName.length()){
						hitWordRatio = (float)hitWordCount(inputCnName, entity_name, false) / entity_name.length();
						hitWordRatio = hitWordRatio < 1 ? 0 : 1;
					}
			...
			if(inputEnName != null && inputEnNameToken > 0){
				int hitCount = hitWordCount(inputEnName, entity_name, true);
				if(inputEnNameToken == entityNameToken){  //當兩邊 Token 相等時		
					if((inputEnNameToken-1) == hitCount && hitCount != 0){  //命中Token為個數減一
						//當條件為 1.個人  2.Token=3，做有順序性差異比對
						if(inputEnNameToken==SwiftMtConst.INCLUSIVE_MATCH_EN_NAME_TOKEN_3 && SwiftMtConst.ENTITY_TYPE_IND.equals(entityType)){ //Token數=3 且為個人，且與名單只有1個Token不同時，處理漢語拼音，有順序性比對方法				
							String [] inputEnNameSpilit = inputEnName.split(" ");
							if(inputEnNameSpilit.length==SwiftMtConst.INCLUSIVE_MATCH_EN_NAME_TOKEN_3){
						...
						}else{ //其它為只做 Token 差異度比對					
							hitWordRatio = compareTokenListDifference(entity_name, inputEnName, DLParameter,
									entityNameToken, inputEnNameToken, hitCount);
						}					
					}else if(inputEnNameToken==SwiftMtConst.INCLUSIVE_MATCH_EN_NAME_TOKEN_4 && hitCount == SwiftMtConst.INCLUSIVE_MATCH_HITWORD_COUNT_2){  //當兩邊Token數各為 4 ，命中Token為個數為2 				
						hitWordRatio = compareTokenListDifference(entity_name, inputEnName, DLParameter,
								entityNameToken, inputEnNameToken, hitCount);						
					}else if(hitCount == inputEnNameToken){ 
						hitWordRatio = (float)hitCount / entityNameToken;
					}
				}else if(((entityNameToken==SwiftMtConst.INCLUSIVE_MATCH_EN_NAME_TOKEN_4 && inputEnNameToken==SwiftMtConst.INCLUSIVE_MATCH_EN_NAME_TOKEN_3)
						||(inputEnNameToken==SwiftMtConst.INCLUSIVE_MATCH_EN_NAME_TOKEN_4 && entityNameToken==SwiftMtConst.INCLUSIVE_MATCH_EN_NAME_TOKEN_3)) 
						&& hitCount==SwiftMtConst.INCLUSIVE_MATCH_HITWORD_COUNT_2){  //當一邊Token數為4，另外一邊為3個Token，且命中Token數為二  
					
					hitWordRatio = compareTokenListDifference(entity_name, inputEnName, DLParameter, entityNameToken,
							inputEnNameToken, hitCount);
	...
	private static float compareTokenListDifference(String entity_name, String inputEnName, int DLParameter,
			int entityNameToken, int inputEnNameToken, int hitCount) {
		float hitWordRatio;
		//1.找出  entityName 不一樣的 Token List
		List<String> DLName1List = new ArrayList<String>();
		String inputEnNameTmp = " "+ inputEnName.trim() + " ";
		String [] entityNameSpilit = entity_name.split(" ");
		for(String tmp : entityNameSpilit){
			if(!inputEnNameTmp.contains(" " + tmp + " ")){
		...
		//2.找出  inputEnName 不一樣的 Token List
		List<String> DLName2List = new ArrayList<String>();
		String entityNameTmp =  " "+ entity_name.trim() + " ";
		String [] inputEnNameSpilit = inputEnName.split(" ");
		for(String tmp : inputEnNameSpilit){
			if(!entityNameTmp.contains(" " + tmp + " ")){
		...
		hitWordRatio = 0;
		int DLDiff = 0;
		for(int i=0;i<DLName1List.size();i++){
			for(int j=0;j<DLName2List.size();j++){
				DLDiff = new DamerauLevenshteinAlgorithm(2,2,2,2).execute(DLName1List.get(i), DLName2List.get(j));
				if(DLDiff <= DLParameter){
					if(entityNameToken > inputEnNameToken){
						hitWordRatio = (float)hitCount / entityNameToken;
					}else{
						hitWordRatio = (float)hitCount / inputEnNameToken;		
					}				
					break;
				}
			}
	...
	/**
	 * 計算命中字數
	 * @param shorterName (smaller)
	 * @param longerName (bigger)
	 * @param isEnglish
	 * @return
	 */
	...
	public static float calculateHitWordRationByCompareTokenListDifference(String entity_name, String inputEnName){
		float result = 0.0f;
		int DLParameter = Integer.parseInt(AmlConfiguration.getString(SwiftMtConst.COM_SAS_DAMERAU_LEVENSHTEIN_PARAMETER));
		int entityNameToken = entity_name.split(" ").length;
		int inputEnNameToken = inputEnName.split(" ").length;
		int hitCount = hitWordCount(inputEnName, entity_name, true);
		if(inputEnNameToken == hitCount){
			result = (float)hitCount/inputEnNameToken;
		}else{
			result = compareTokenListDifference(entity_name, inputEnName, DLParameter, entityNameToken, inputEnNameToken, hitCount);
		}
		logger.debug("entityNameToken : " + entityNameToken + ", inputEnNameToken : " + inputEnNameToken + ", hitCount : " + hitCount + ", result : " + result);
		return result;
	}
	
	public static boolean isFulFillDifference(String entity_name, String inputEnName){
		boolean result = false;
		float hitWordRatio = Float.parseFloat(AmlConfiguration.getString(SwiftMtConst.COM_SAS_DAMERAU_LEVENSHTEIN_SWIFTCHECK_HITWORDRATIO));
		float diff = calculateHitWordRationByCompareTokenListDifference(entity_name, inputEnName);
		logger.debug(String.format("entity_name : %s, inputEnName : %s, hitWordRatio : %s, diff : %s", entity_name, inputEnName, hitWordRatio, diff));
		if(diff < hitWordRatio){
			result = true;
		}
		return result;
	}
	...
	public static void main(String[] args) {
		// calculateHitWordRatio(String entity_name, String inputCnName, String inputEnName, int DLParameter , String entityType){
		float hitWordRatio = NameCheckUtil.calculateHitWordRatio("JEAN CHARLES PAYL ABC","","JEAN CHARLES FDAEL AEC", 2 , "02");
//		float hitWordRatio2 = NameCheckUtil.calculateHitWordRatio("abc def a ghi","","dfeg a ghi abc", 4);
